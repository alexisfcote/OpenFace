...
namespace UtilitiesOF {
	...
	public:
		RecorderOpenFaceParameters(bool sequence, bool is_from_webcam, bool output_2D_landmarks, bool output_3D_landmarks,
			...
			bool output_aligned_faces, bool record_bad_aligned, float fx, float fy, float cx, float cy, double fps_vid_out, int streaming_port)
		{
			...
			m_params = new Utilities::RecorderOpenFaceParameters(sequence, is_from_webcam, 
				...
				output_gaze, output_hog, output_tracked, output_aligned_faces, record_bad_aligned, fx, fy, cx, cy, fps_vid_out, streaming_port);
